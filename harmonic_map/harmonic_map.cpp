    ...
    map<int, CPoint> &fixedvertex)
{
    ...
    cout << "Initialize all vertices to (0,0), except for once not fixed and not on boundary" << endl;
    ...
    while (it_vertex != mesh.vertices().end())
    {
        ...
        if (it_mapfixed != fixedvertex.end())
        {
            ...
            cout << "Fixed ID:" << vertex->id() << "; " << vertex->m_mapx << ", " << vertex->m_mapy << endl;
        }
    ...
    cout << "Harmonic Energy being computed" <<endl;
    double e0 = 0;
...
void morph_map(CMesh &mesh1, CMesh &mesh2, map<int, int> &fixed_pair)
{
    ...
    while (it_fixed_pair != fixed_pair.end())
    {
        ...
        cout << "Morph Fixed Points: " << vertex_id_mesh2 << "; " << fixed2d.v[0] << ", " << fixed2d.v[1] << endl;
        it_fixed_pair++;
    }
